...
impl EventHandler for Handler {
    async fn ready(&self, ctx: Context, ready: Ready) {
        ...
        check_msg(channel_id.say(&ctx.http, "Connected!").await);
        ...
        if let Some(handler_lock) = manager.get(guild_id) {
            ...
            check_msg(channel.id().say(&ctx.http, "Playing song").await);
        } else {
            check_msg(channel.id().say(&ctx.http, "Not in a voice channel to play in").await);
        }
    }
}
